    ...
    <div className="h-full overflow-y-auto p-6 no-scrollbar">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-ink-muted">
            Phishing-resistant sign-ins synced across your devices
          </p>
        </div>
        <button
          onClick={() => {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {passkeys.map((p) => (
          <div
            key={p.id}
            className="rounded-2xl hairline bg-surface p-4 hover:shadow-float transition"
          >
            <div className="flex items-center gap-3">
              <div className="size-11 rounded-xl brand-gradient-soft grid place-items-center">
              ...
              <button
                ...
                className="size-8 rounded-md hover:bg-muted grid place-items-center text-destructive"
              >
            ...
            <div className="mt-3 flex items-center gap-2 text-[11px]">
              {p.biometric && (
                <span className="px-2 py-0.5 rounded-md bg-accent text-brand-ink inline-flex items-center gap-1">
                  <ShieldCheck className="size-3" />
                  Biometric
                </span>
              )}
              {p.synced ? (
                <span className="px-2 py-0.5 rounded-md bg-success/15 text-success">Synced</span>
              ) : (
                <span className="px-2 py-0.5 rounded-md hairline text-ink-muted">Local only</span>
              )}
            </div>
          </div>
        ))}
