...
export function ShareModal() {
  ...
  const send = () => {
    ...
    invite({
      itemTitle: shareTarget,
      sharedWith: email,
      role,
      expiresAt: temp ? Date.now() + 7 * 86_400_000 : undefined,
    });
      ...
      <div className="w-full max-w-md glass-strong rounded-2xl shadow-float overflow-hidden animate-scale-in">
        ...
        <div className="p-5 space-y-3">
          ...
          <div className="flex gap-2">
            {(["viewer", "editor"] as const).map((r) => (
              <button
                key={r}
                onClick={() => setRole(r)}
                className={`flex-1 h-9 rounded-lg text-sm capitalize ${role === r ? "bg-brand text-brand-foreground" : "hairline"}`}
              >
                {r}
              </button>
            ))}
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={closeShare} className="h-10 px-3 rounded-lg hairline text-sm">
              Cancel
            </button>
            <button
              onClick={send}
