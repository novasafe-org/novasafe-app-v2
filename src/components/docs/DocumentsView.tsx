  ...
  const onDrop = (e: React.DragEvent) => {
    ...
    files.forEach((f) =>
      addDocument({
        ...
        folder: "Uploads",
        size: f.size,
        kind: f.type.includes("image") ? "image" : "pdf",
        shared: [],
        tags: [],
        versions: 1,
      }),
      ...
      <div className="w-48 shrink-0 p-4 border-r border-hairline space-y-1">
        ...
        {folders.map((f) => (
          <button
            key={f}
            onClick={() => setFolder(f)}
            className={`w-full text-left text-sm px-2.5 py-1.5 rounded-lg ${folder === f ? "bg-accent text-brand-ink" : "text-ink-muted hover:bg-muted"}`}
          >
            {f}
          </button>
        ))}
      ...
      >
          ...
          <label className="h-9 px-3 rounded-lg bg-brand text-brand-foreground text-sm inline-flex items-center gap-1.5 shadow-float cursor-pointer">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (files.length) toast.success(`${files.length} uploaded`);
              }}
            />
          </label>
        </div>
        <div className="flex-1 overflow-y-auto px-4 pb-4 no-scrollbar">
          <ul className="space-y-1">
            {filtered.map((d) => (
              <li key={d.id}>
                <button
                  ...
                  className={`w-full text-left flex items-center gap-3 p-2.5 rounded-xl ${selected?.id === d.id ? "bg-accent" : "hover:bg-muted/70"}`}
                >
                  <span className="size-10 rounded-lg brand-gradient-soft grid place-items-center text-brand">
                    {d.kind === "image" ? (
                  ...
                  <span className="flex-1 min-w-0">
                    ...
                    <span className="block text-xs text-ink-muted">
                      {d.folder} · {fmt(d.size)} · v{d.versions}
                    </span>
                  </span>
                  {d.shared.length > 0 && (
            ...
            {filtered.length === 0 && (
              <li className="text-center py-12 text-sm text-ink-muted">No documents</li>
            )}
          </ul>
        </div>
      </div>
      {selected && (
        <div className="hidden lg:flex w-[320px] shrink-0 border-l border-hairline flex-col bg-surface/40">
          <div className="p-5 border-b border-hairline">
            <div className="size-12 rounded-xl brand-gradient-soft grid place-items-center mb-3 text-brand">
              {selected.kind === "image" ? (
            ...
            <div className="text-xs text-ink-muted">
              {selected.folder} · {fmt(selected.size)}
            </div>
          </div>
          <div className="p-5 space-y-2 text-xs">
            ...
            <div className="flex justify-between">
              <span className="text-ink-muted">Shared with</span>
              <span>{selected.shared.length || "—"}</span>
            </div>
            <div className="flex justify-between">
          ...
          <div className="mt-auto p-4 grid grid-cols-3 gap-2">
            <button
              ...
              className="h-9 rounded-lg hairline text-xs inline-flex items-center justify-center gap-1"
            >
              <Download className="size-3.5" />
              Get
            </button>
            <button
              onClick={() => openShare(selected.name)}
              className="h-9 rounded-lg hairline text-xs inline-flex items-center justify-center gap-1"
            >
            ...
            <button
              ...
              className="h-9 rounded-lg text-xs inline-flex items-center justify-center gap-1 text-destructive hover:bg-destructive/10"
            >
