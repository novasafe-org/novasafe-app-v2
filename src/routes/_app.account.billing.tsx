      ...
      <div className="p-6 max-w-3xl space-y-4">
        ...
        <div className="relative overflow-hidden rounded-3xl p-6 text-white brand-gradient shadow-float">
          ...
          <div className="relative">
            ...
            <div className="text-sm opacity-80 mt-1">Unlimited items · Encrypted sharing · Priority sync</div>
            <div className="mt-5 flex gap-2">
              {(["Free","Pro","Family"] as const).map(p => (
                <button key={p} onClick={()=>{setPlan(p);toast.success(`Switched to ${p}`);}} className={`h-9 px-3 rounded-lg text-sm ${plan===p?"bg-white text-brand-ink":"bg-white/15 hover:bg-white/25"}`}>{p}</button>
              ))}
            </div>
          </div>
        </div>
            ...
            <tbody>
              {invoices.map(inv=>(
                <tr key={inv.id} className="border-t border-hairline first:border-t-0">
                  ...
                  <td className="p-3 text-ink-muted">{new Date(inv.paidAt).toLocaleDateString()}</td>
                  <td className="p-3 mono">${inv.amount.toFixed(2)}</td>
                  <td className="p-3"><span className="text-xs px-2 py-0.5 rounded-md bg-success/15 text-success capitalize">{inv.status}</span></td>
                  <td className="p-3 text-right"><button onClick={()=>toast.success("Invoice downloaded")} className="text-xs text-brand hover:underline">Download</button></td>
                </tr>
              ))}
            </tbody>
          </table>
